...
final class SuiteIntegration {
 ...
 public function register():void{$this->core->events()->listen('relationship.changed',array($this,'relationship_changed'));add_action('save_post',array($this,'content_changed'),200,3);add_action('edited_term',fn($id)=>$this->dispatch_object('term',$id),200);add_action('profile_update',fn($id)=>$this->dispatch_object('user',$id),200);add_action('cgm_core/register_suite_integrations',$this->callable());}
 private function callable():callable{return function(){};}
