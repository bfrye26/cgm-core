<?php
namespace CGM\Core\REST;use CGM\Core\Plugin;use CGM\Core\Query\{QueryResult,SavedQueryRepository};
final class QueryController extends BaseController {
    ...
    public function register_routes():void{
        ...
        register_rest_route($this->namespace,'/queries/(?P<id>[a-zA-Z0-9_-]+)/delete',array('methods'=>\WP_REST_Server::CREATABLE,'callback'=>array($this,'delete'),'permission_callback'=>fn()=>current_user_can('manage_cgm_queries'),'args'=>array('id'=>array('required'=>true,'type'=>'string','sanitize_callback'=>'sanitize_text_field'))));
        register_rest_route($this->namespace,'/queries/(?P<id>[a-zA-Z0-9_-]+)/clone',array('methods'=>\WP_REST_Server::CREATABLE,'callback'=>array($this,'clone'),'permission_callback'=>fn()=>current_user_can('manage_cgm_queries'),'args'=>array('id'=>array('required'=>true,'type'=>'string','sanitize_callback'=>'sanitize_text_field'))));
