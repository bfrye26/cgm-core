...
final class SavedQueryRepository {
    ...
    public function find(string|int $id):?array{if(is_string($id)){if(str_starts_with($id,'code:'))$id=substr($id,5);$slug=sanitize_title($id);if(isset($this->code[$slug]))return $this->code[$slug];}$post=is_numeric($id)?get_post((int)$id):get_page_by_path(sanitize_title((string)$id),OBJECT,self::POST_TYPE);if(!$post||self::POST_TYPE!==$post->post_type)return null;$def=get_post_meta($post->ID,'_cgm_query_definition',true);return array('id'=>$post->ID,'slug'=>$post->post_name,'title'=>$post->post_title,'public'=>(bool)get_post_meta($post->ID,'_cgm_query_public',true),'definition'=>is_array($def)?$def:array(),'managed_by'=>'database','readonly'=>false,'source'=>'wordpress');}
