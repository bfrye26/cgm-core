...
final class Commands {
    ...
    private function config_export(array $assoc):void{$json=wp_json_encode($this->core->configuration()->export(),JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);if(!empty($assoc['file'])){$file=(string)$assoc['file'];$dir=realpath(dirname($file));if(false===$dir||!str_starts_with(wp_normalize_path($dir),wp_normalize_path(WP_CONTENT_DIR))){\WP_CLI::error('Configuration export path must be inside the content directory.');}if(false===file_put_contents($file,$json))\WP_CLI::error('Could not write configuration file.');\WP_CLI::success('Configuration exported.');return;}\WP_CLI::line($json);}
