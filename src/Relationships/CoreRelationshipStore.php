...
class CoreRelationshipStore implements QueryableRelationshipStoreInterface {
    ...
    private function property_expression(string $property,string $alias):string{$p=sanitize_key(str_replace(array('meta.','meta:'),'',$property));if(''===$p)return 'NULL';return match($property){'role'=>"{$alias}.role",'primary','is_primary'=>"{$alias}.is_primary",'order','sort_order'=>"{$alias}.sort_order",'target','target_id'=>"{$alias}.target_id",'source','source_id'=>"{$alias}.source_id",default=>"JSON_UNQUOTE(JSON_EXTRACT({$alias}.meta, '$.\"{$p}\"'))"};}
